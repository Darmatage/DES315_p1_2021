...
public class RyanGarvanDoorMimicMove : MonoBehaviour
{
    ...
    public float m_eyeTurnSpeed = Mathf.PI / 1.5f; // The speed at which the door looks around when it can't see the player
    ...
    void Update()
    {
        ...
        else
        {
            ...
            else
            {
                if (!can_see_player)
                {
                    m_lookAngle += m_eyeTurnSpeed * Time.deltaTime;
                }
                else
                {
