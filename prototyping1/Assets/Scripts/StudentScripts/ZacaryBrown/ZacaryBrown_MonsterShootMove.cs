...
//Copied from the teacher to enable bullet damage
public class ZacaryBrown_MonsterShootMove : MonoBehaviour {
	...
	void Update () {
		if (player != null && Vector2.Distance (transform.position, player.position) < shootDist) {
			...
			if (timeBtwShots <= 0 && Vector2.Distance(transform.position, player.position) < shootDist) {
